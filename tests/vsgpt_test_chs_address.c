...
#include "../libvsgpt/libvsgpt_chs_address.h"

uint8_t vsgpt_test_chs_address_byte_stream[ 3 ] = {
	0x01, 0x02, 0x03 };

#if defined( __GNUC__ ) && !defined( LIBVSGPT_DLL_IMPORT )

/* Tests the libvsgpt_chs_address_initialize function
 * Returns 1 if successful or 0 if not
 */
int vsgpt_test_chs_address_initialize(
     void )
{
	...
	/* Test regular cases
	 */
	result = libvsgpt_chs_address_initialize(
	          &chs_address,
	          &error );
	...
	result = libvsgpt_chs_address_free(
	          &chs_address,
	          &error );
	...
	chs_address = (libvsgpt_chs_address_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvsgpt_chs_address_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libvsgpt_chs_address_initialize with memset failing
		 */
...
/* Tests the libvsgpt_chs_address_free function
 * Returns 1 if successful or 0 if not
 */
int vsgpt_test_chs_address_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
/* Tests the libvsgpt_chs_address_copy_from_byte_stream function
 * Returns 1 if successful or 0 if not
 */
int vsgpt_test_chs_address_copy_from_byte_stream(
     void )
{
	libcerror_error_t *error          = NULL;
	libvsgpt_chs_address_t *chs_address = NULL;
	 ...
	 */
	result = libvsgpt_chs_address_copy_from_byte_stream(
	          chs_address,
	          vsgpt_test_chs_address_byte_stream,
	          3,
	          LIBVSGPT_ENDIAN_BIG,
	          &error );
...
#endif
{
	...
	VSGPT_TEST_RUN(
	 "libvsgpt_chs_address_initialize",
	 vsgpt_test_chs_address_initialize );

	VSGPT_TEST_RUN(
	 "libvsgpt_chs_address_free",
	 vsgpt_test_chs_address_free );

	VSGPT_TEST_RUN(
	 "libvsgpt_chs_address_copy_from_byte_stream",
	 vsgpt_test_chs_address_copy_from_byte_stream );
