...
void (*vsgpttools_signal_signal_handler)( vsgpttools_signal_t ) = NULL;
             ...
             vsgpttools_signal_t signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( vsgpttools_signal_signal_handler != NULL )
			{
				vsgpttools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "vsgpttools_signal_attach";
...
#if defined( _MSC_VER )
	vsgpttools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	vsgpttools_signal_signal_handler = NULL;
